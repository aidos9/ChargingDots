...
-(void) updateBarPercentage {
  ...
  CGRect fillBarRect = CGRectMake(0,0,self->bar.frame.size.width * DEBUG_BATTERY_PERCENTAGE,height);
  #else
  CGRect fillBarRect = CGRectMake(0,0,self->bar.frame.size.width * [UIDevice currentDevice].batteryLevel,height);
  #endif
...
-(void) lengthChanged {
  ...
  if (self->lengthConstant != [PreferencesManager parentViewLength]) {
    ...
    CGRect newPosition = CGRectMake(self.frame.origin.x, self.frame.origin.y, width, self.frame.size.height);
    self.frame = newPosition;
    [self relayout];
    [self xChanged]; // We call this here because we want to ensure that since the center position has changed that we are still centered.
  }
}
...
-(void) layoutDots {
  ...
  CGFloat dotDiameter = self.frame.size.height*0.95*[PreferencesManager dotRadius]; // Since dotRadius is a percentage value anyway
  CGFloat spacing = (self.frame.size.width - dotDiameter * numberOfDots) / (numberOfDots+1); // +1 to account for spacing after the last dot
  int dotIndex = 0;
  ...
  while(self->dots.count < numberOfDots) {
    ...
    CGRect frame = CGRectMake(spacing + spacing * dotIndex + dotIndex * dotDiameter, self.frame.size.height/2 - dotDiameter/2, dotDiameter, dotDiameter);
    UIView* newView = [[UIView alloc] initWithFrame: frame];
    newView.layer.cornerRadius = dotDiameter/2;
    newView.layer.masksToBounds = true;
    [self addSubview: newView];
    // Here we append because unlike the vertical layout the last view we add is the last view we want coloured
...
-(void) layoutBar {
  ...
  CGFloat height = self.frame.size.height * [PreferencesManager dotRadius];
  CGRect mainBarRect  = CGRectMake(0,self.frame.size.height/2 - height/2,self.frame.size.width,height);
