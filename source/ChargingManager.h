...
// This interface is the overall manager, it is responsible for handling Darwin
// notifications(created when a setting is changed) and passing that to the parentview. It is also
// responsible for hiding/showing and changing the orientation of the parentView.
...
// This method handles the selection of which *Changed method should be called, which in turn call a
// method on the parentView to handle the change in the settings
- (void)notificationCallback:(NSString*)name;

// These methods fade in/out the parentView and also do a sanity check to ensure that parentView is
// not nil.
- (void)showParentView;
- (void)hideParentView;

// Checks if we are charging or if we should show the view otherwise and then shows or hides
// parentView
- (void)checkShowView;
