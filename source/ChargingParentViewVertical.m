...
// Change the height and y-position based on the amount of charge we have.
  ...
  CGRect fillBarRect = CGRectMake(0, self->bar.frame.size.height * (1 - DEBUG_BATTERY_PERCENTAGE),
                                  width, self->bar.frame.size.height * DEBUG_BATTERY_PERCENTAGE);
#else
  CGRect fillBarRect =
      CGRectMake(0, self->bar.frame.size.height * (1 - [UIDevice currentDevice].batteryLevel),
                 width, self->bar.frame.size.height * [UIDevice currentDevice].batteryLevel);
#endif
...
- (void)lengthChanged {
  ...
  if (self->lengthConstant != [PreferencesManager parentViewLength]) {
    ...
    CGRect newPosition =
        CGRectMake(self.frame.origin.x, self.frame.origin.y, self.frame.size.width, length);
...
- (void)layoutDots {
  ...
  CGFloat spacing = (self.frame.size.height - dotDiameter * numberOfDots) /
  ...
  while (self->dots.count < numberOfDots) {
    ...
    // previous dots.
    CGRect frame =
        CGRectMake(self.frame.size.width / 2 - dotDiameter / 2,
                   spacing + spacing * dotIndex + dotIndex * dotDiameter, dotDiameter, dotDiameter);
    UIView* newView = [[UIView alloc] initWithFrame:frame];
    newView.layer.cornerRadius = dotDiameter / 2;
    newView.layer.masksToBounds = true;
    [self addSubview:newView];
    // We have to insert at 0 because as we create the views we move down but we want the top view
    // to be the last one to be filled in and hence it must be at the last index.
...
- (void)layoutBar {
  ...
  CGRect mainBarRect =
      CGRectMake(self.frame.size.width / 2 - width / 2, 0, width, self.frame.size.height);
