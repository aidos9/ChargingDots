...
// This interface is the base for the parent view which houses and manages the visual appearance of the widget, subclasses are used to layout in different orientations.
@interface ChargingParentViewBase : UIView {
  ...
  UIView* barFill;
  // Track the pulsing index so we don't create two animations and so we can remove the animation on demand.
  int pulsingIndex;
  // The animation.
...
// These methods recolor the dots or the bar. Forcing only affects the dots in that it overrides the check if the number of dots colored has changed
...
// Create the pulse animation for bar mode, for dot mode this is handled in the updateViewColorsDots method.
